...
### İnsula (Ada Lobu)  `insula`

**Z-Anatomy obje(ler)i:** _(yok)_

> Atlasta bu 'bütün organ/lob' objesi (.j uzantılı) 2 vertex'lik boş bir yer tutucu (muhtemelen sadece organizasyonel/etiket amaçlı); gerçek görsel yüzey, bu düğümün altındaki alt-bölgelerin (gyruslar/alt-yapılar) birleşiminden oluşuyor. Bu yüzden kendi mesh'i yok, JSON'da sadece üst-grup etiketi.

### Limbik Lob (Kortikal)  `limbik-lob`

**Z-Anatomy obje(ler)i:** _(yok)_

> TA'da tek bir mesh yok; singulat korteks + parahipokampal girus + retrosplenyal korteks alt-yapılarının kavramsal grubu, üst düğüm (parent) olarak kullanılacak, kendi mesh'i yok.
...
> Kaudat çekirdek + putamen + globus pallidus + substantia nigra + subtalamik çekirdek + nucleus accumbensin kavramsal grubu; kendi mesh'i yok.

### Limbik Sistem  `limbik-sistem`

**Z-Anatomy obje(ler)i:** _(yok)_

> Hipokampus + amigdala + fornix + mamiller cisimler + septal çekirdeklerin kavramsal grubu; kendi mesh'i yok.

### Diensefalon (Ara Beyin)  `diensefalon`

**Z-Anatomy obje(ler)i:** _(yok)_

> Talamus + hipotalamus + epitalamus + hipofiz bezinin kavramsal grubu; kendi mesh'i yok.

### Hipofiz Bezi  `hipofiz-bezi`
...
> Fonksiyonel bir bölge, TA'da karşılığı yok; Frontal lobe mesh'i üzerinde üst-yan bölgede şematik alt-alan olarak işaretlenebilir.

### Ventromedial Prefrontal Korteks  `ventromedial-prefrontal-korteks`
...
> Bu atlasta ayrı bir karşılığı yok; corpus callosumun splenium'una yakın, singulat korteksin en arka ucunda şematik nokta olarak işaretlenmesi gerekecek.
...
> Atlasta hiç yok; kaudat başı ile putamenin birleştiği ventral striatum bölgesinde şematik nokta olarak işaretlenmesi gerekecek.

### Substantia Nigra  `substantia-nigra`

**Z-Anatomy obje(ler)i:** _(yok)_

> Atlasta hiç yok; orta beyin (midbrain) mesh'i içinde, anatomik literatüre göre şematik nokta olarak işaretlenmesi gerekecek.

### Subtalamik Çekirdek  `subtalamik-cekirdek`
...
> Atlasta hiç yok (dentat, fastigial, emboliform, globose çekirdekleri bulunamadı); serebellum mesh'i içinde 4. ventrikül tabanına yakın şematik nokta olarak işaretlenmesi gerekecek.

### İç Kapsül  `ic-kapsul`

**Z-Anatomy obje(ler)i:** _(yok)_

> Atlasta hiç yok; talamus ile bazal ganglionlar arasındaki V biçimli bölgede şematik olarak işaretlenmesi ya da basit bir geometriyle temsil edilmesi gerekecek.

### Arkuat Fasikül  `arkuat-fasikul`

**Z-Anatomy obje(ler)i:** _(yok)_

> Atlasta hiç yok (difüzyon-MRI traktografi verisi gerektirir); Broca-Wernicke alanları arasında basitleştirilmiş bir eğri/tüp olarak şematik gösterilmesi gerekecek.
